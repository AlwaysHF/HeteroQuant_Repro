...
class OlmoeOutlierExpertBank(nn.Module):
...
class QuantOlmoeAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.q_norm = DuOlmoeRMSNorm(org_module.q_norm,eps=org_module.q_norm.variance_epsilon)
        self.k_norm = DuOlmoeRMSNorm(org_module.k_norm,eps=org_module.k_norm.variance_epsilon)
...
class QuantOlmoeSparseMoeBlock(nn.Module):
    def __init__(
        ...
        self.outlier_expert = (
            OlmoeOutlierExpertBank(gate_outlier_modules, up_outlier_modules, gate_outlier_indices, up_outlier_indices, args=args)
            if outlier_topk > 0
            else None
        )
...
class QuantOlmoeDecoderLayer(nn.Module):
    def __init__(self, 
        ...
        self.mlp = QuantOlmoeSparseMoeBlock(
            ...
            args=args,
            layer_idx=layer_idx)
        self.input_layernorm = DuOlmoeRMSNorm(ori_layer.input_layernorm,eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = DuOlmoeRMSNorm(ori_layer.post_attention_layernorm,eps=ori_layer.post_attention_layernorm.variance_epsilon)
