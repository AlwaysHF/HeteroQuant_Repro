...
MID_LINE_ALPHA = 0.28
EXTREME_LINE_ALPHA = 0.98
EXTREME_MARKER_SIZE = 28.0
EXTREME_MARKER_HALO_SIZE = 46.0
...
def plot_exposure_curve(ax, x, y, color, linestyle, label, high=HIGH_THRESHOLD, low=LOW_THRESHOLD):
    """Fade the near-mean section and emphasize the high/low sections."""
    x = np.asarray(x)
    y = np.asarray(y, dtype=float)
    extreme_mask = (y > high) | (y < low)

    # Draw the complete curve faintly so the trajectory stays continuous.
    ax.plot(
        x,
        y,
        color=color,
        ...
        linestyle=linestyle,
        alpha=MID_LINE_ALPHA,
        zorder=2,
        label=label,
    )

    # Repaint only the high/low-exposure segments with the saturated color.
    y_extreme = np.where(extreme_mask, y, np.nan)
    ax.plot(
        x,
        y_extreme,
        color=color,
        linewidth=CURVE_LINEWIDTH + 0.12,
        linestyle=linestyle,
        alpha=EXTREME_LINE_ALPHA,
        zorder=3,
        label="_nolegend_",
    )
...
def finish_axes(ax, legend_loc="lower left"):
    ...
    # Keep legend samples saturated even though near-mean curve sections are faded.
