...
# - Near-mean curve sections are faded; high/low experts are highlighted
# =====================================================
HIGH_THRESHOLD = 1.5
LOW_THRESHOLD = 0.5
CURVE_LINEWIDTH = 1.20
# Amount mixed with white for curve portions inside 0.5x--1.5x.
# Larger values make the middle-region line lighter.
MID_LIGHTEN_AMOUNT = 0.68
EXTREME_MARKER_SIZE = 32.0
EXTREME_MARKER_HALO_SIZE = 50.0
EXTREME_MARKER_WIDTH = 0.95
...
def darken_color(color, factor=0.72):
    """Return a slightly darker RGB version of a Matplotlib color."""
...
def lighten_color(color, amount=MID_LIGHTEN_AMOUNT):
    """Blend a color with white; amount=0 keeps it unchanged."""
    rgb = np.asarray(matplotlib.colors.to_rgb(color))
    white = np.ones(3)
...
def add_extreme_markers(ax, x, y, color, high=HIGH_THRESHOLD, low=LOW_THRESHOLD):
    """Use saturated filled circles to highlight high/low-exposure experts."""
    ...
    # A white halo separates the highlighted experts from the colored bands
    # and from nearby curves.
    ax.scatter(
        x[mask],
        y[mask],
...
def split_curve_by_regions(x, y, low, high, scale="log"):
    """Split a polyline exactly where it crosses low/high horizontal bands.

    Matplotlib draws straight segments in transformed coordinates. Therefore,
    for a log-y axis, intersections are computed in log(y), not directly in y.

    Returns a list of ``(region, xs, ys)`` where region is ``middle`` or
    ``extreme``. Adjacent subsegments in the same region are merged.
    """
    ...
    for i in range(len(x) - 1):
        ...
        for ta, tb in zip(cuts[:-1], cuts[1:]):
            ...
            zb = z0 + tb * (z1 - z0)
            zmid = 0.5 * (za + zb)
            region = "middle" if low_z <= zmid <= high_z else "extreme"
...
def plot_exposure_curve(
    ...
    """Lighten only the curve portions inside the middle exposure band."""
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    middle_color = lighten_color(color)

    # Do not draw a faint full-length underlay. Instead, split every segment at
    # 0.5x and 1.5x, then color each resulting portion independently.
    for region, xs, ys in split_curve_by_regions(x, y, low, high, scale=scale):
        ax.plot(
            xs,
            ys,
            color=middle_color if region == "middle" else color,
            linewidth=CURVE_LINEWIDTH if region == "middle" else CURVE_LINEWIDTH + 0.12,
            linestyle=linestyle,
            alpha=1.0,
    ...
    # Dedicated saturated legend handle.
    ax.plot(
        [],
        [],
        color=color,
    ...
    add_extreme_markers(ax, x, y, color, high=high, low=low)
...
def finish_axes(ax, legend_loc="lower left"):
    ...
    # Legend handles are already dedicated saturated samples.
