...
def top10_gap_by_layer(df):
    ...
    for layer, group in df.groupby("layer_index"):
        ...
        vals = np.sort(vals)[::-1]
        top10 = float(vals[:10].mean())
        bottom10 = float(vals[-10:].mean())
        rows.append(
            {
                "layer": int(layer),
                "top10_mean": top10,
                "bottom10_mean": bottom10,
...
def plot_gap_panel(ax, olmoe_df, qwen_df):
    olmoe = top10_gap_by_layer(olmoe_df)
    qwen = top10_gap_by_layer(qwen_df)
    olmoe_mean = float(olmoe["top10_bottom10_gap"].mean())
    qwen_mean = float(qwen["top10_bottom10_gap"].mean())
    ax.plot(
        olmoe["layer"],
        olmoe["top10_bottom10_gap"],
    ...
    ax.set_ylabel("Top10 mean - Bottom10 mean")
    ...
    add_panel_caption(ax, "(c) Top-bottom exposure gap")
    return pd.concat(
        [
            olmoe.assign(model="OLMoE"),
            qwen.assign(model="Qwen"),
        ],
        ignore_index=True,
    )
...
def main():
    ...
    summary = plot_gap_panel(axes[2], olmoe_df, qwen_df)
    fig.tight_layout(w_pad=1.05, pad=0.45, rect=(0.0, 0.10, 1.0, 0.98))

    png = OUT_DIR / "routing_exposure_triptych_top10_gap.png"
    pdf = OUT_DIR / "routing_exposure_triptych_top10_gap.pdf"
    csv = OUT_DIR / "routing_exposure_triptych_top10_gap_summary.csv"
