...
def gradient_band_log(ax, y0, y1, color, alpha0, alpha1, steps=48):
    y0 = max(y0, 1e-6)
    y1 = max(y1, y0 * 1.001)
    edges = np.exp(np.linspace(math.log(y0), math.log(y1), steps + 1))
    for i in range(steps):
        alpha = alpha0 + (alpha1 - alpha0) * (i / max(steps - 1, 1))
        ax.axhspan(edges[i], edges[i + 1], color=color, alpha=alpha, linewidth=0, zorder=-10)


def add_log_bands(ax, ymin, ymax):
    gradient_band_log(ax, 1.5, ymax, "#DDEBFF", 0.10, 0.28)
    gradient_band_log(ax, 0.5, 1.5, "#EEF2F7", 0.10, 0.20)
    gradient_band_log(ax, ymin, 0.5, "#FFE9D8", 0.24, 0.10)
    ax.axhline(1.0, color="#7C8491", linewidth=1.0, linestyle=(0, (4, 3)), label="Layer mean", zorder=-1)
    ax.axhline(1.5, color="#FFFFFF", linewidth=1.1, alpha=0.95, zorder=-1)
    ax.axhline(0.5, color="#FFFFFF", linewidth=1.1, alpha=0.95, zorder=-1)
...
def add_log2_bands(ax, ymin, ymax):
    ...
    hi = math.log2(1.5)
    lo = math.log2(0.5)
    for y0, y1, color, alpha in [
        (hi, ymax, "#DDEBFF", 0.30),
        (lo, hi, "#EEF2F7", 0.24),
        (ymin, lo, "#FFE9D8", 0.26),
    ]:
        ax.axhspan(y0, y1, color=color, alpha=alpha, linewidth=0, zorder=-10)
    ax.axhline(0.0, color="#7C8491", linewidth=1.0, linestyle=(0, (4, 3)), label="Layer mean", zorder=-1)
    ax.axhline(hi, color="#FFFFFF", linewidth=1.1, alpha=0.95, zorder=-1)
    ax.axhline(lo, color="#FFFFFF", linewidth=1.1, alpha=0.95, zorder=-1)
    tx = ax.get_yaxis_transform()
