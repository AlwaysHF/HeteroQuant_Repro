...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/allocation_metric_ablation_olmoe_ppl_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[alloc-ablation] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
  cp -a "$FALLBACK_CACHE_DIR/$planner_cache_name" "$CACHE_DIR/"
fi

mkdir -p "$OUT_ROOT"
SUMMARY_CSV="$OUT_ROOT/summary.csv"
printf 'label,weight_score_coef,route_alpha,score_transform,tier_quants,tier_fractions,actual_avg_sum_bits,status,wikitext2,c4,calibration_seconds,duquant_seconds,inference_wikitext2_seconds,inference_c4_seconds,plan_path,output_dir,case_log\n' > "$SUMMARY_CSV"

CASES=(
  "no_weight_half|1.0|0.5|log"
  "no_route_half|0.5|1.0|log"
  "no_both_halves|1.0|1.0|log"
  "no_log_no_halves|1.0|1.0|raw"
)
...
append_summary() {
  local label="$1"
  local weight_score_coef="$2"
  local route_alpha="$3"
  local score_transform="$4"
...

  {
    ...
    csv_quote "$score_transform"; printf ','
...
echo "[alloc-ablation] output root: $OUT_ROOT"
echo "[alloc-ablation] test_dataset: $TEST_DATASET"
echo "[alloc-ablation] tasks: ${TASKS:-<none>}"
echo "[alloc-ablation] tier_quants: $TIER_QUANTS"
echo "[alloc-ablation] tier_fractions: $TIER_FRACTIONS"
echo "[alloc-ablation] weight_channel_group_size: $WEIGHT_CHANNEL_GROUP_SIZE"

for idx in "${!CASES[@]}"; do
  IFS='|' read -r label weight_score_coef route_alpha score_transform <<< "${CASES[$idx]}"
  case_num=$((idx + 1))
  ...
  echo
  echo "[alloc-ablation] case $case_num/$total: $label"
  echo "[alloc-ablation] weight_score_coef=$weight_score_coef route_alpha=$route_alpha score_transform=$score_transform"
  ...
  append_summary "$label" "$weight_score_coef" "$route_alpha" "$score_transform" "$actual_avg_sum_bits" "$status" \
  ...
  echo "[alloc-ablation] finished $label with status=$status"
  echo "[alloc-ablation] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[alloc-ablation] summary: $SUMMARY_CSV"
done

echo
echo "[alloc-ablation] all done: $SUMMARY_CSV"
