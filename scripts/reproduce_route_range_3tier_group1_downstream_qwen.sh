...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/reproduce_route_range_3tier_group1_downstream_qwen_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[qwen-route-range-3tier-g1] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
...
echo "[qwen-route-range-3tier-g1] output root: $OUT_ROOT"
echo "[qwen-route-range-3tier-g1] model: $MODEL"
echo "[qwen-route-range-3tier-g1] model_name: $MODEL_NAME"
echo "[qwen-route-range-3tier-g1] formula: route_range = (input_range + down_range + weight_range) * route_norm"
echo "[qwen-route-range-3tier-g1] tier_quants: $TIER_QUANTS"
echo "[qwen-route-range-3tier-g1] tier_fractions: $TIER_FRACTIONS"
echo "[qwen-route-range-3tier-g1] weight_channel_group_size: $WEIGHT_CHANNEL_GROUP_SIZE"
echo "[qwen-route-range-3tier-g1] OSE: topk=$MOE_OUTLIER_TOPK quant=$MOE_OUTLIER_QUANT score=$MOE_OUTLIER_SCORE"
echo "[qwen-route-range-3tier-g1] test_dataset: $TEST_DATASET"
echo "[qwen-route-range-3tier-g1] tasks: $TASKS"
...
if [[ "$status" == "ok" ]]; then
  if ! run_logged "$RUN_LOG" \
    env PLAN_PATH="$PLAN_PATH" \
        ...
        bash scripts/reproduce_qwen_744.sh; then
    status="run_failed"
  fi
fi
...
{
  csv_quote "qwen_3tier_w4a4_w5a8_w6a8"; printf ','
...
echo "[qwen-route-range-3tier-g1] done with status=$status"
echo "[qwen-route-range-3tier-g1] summary: $SUMMARY_CSV"
