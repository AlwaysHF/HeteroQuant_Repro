...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/ose_topk_routed_tail_olmoe_ppl_group2048_${RUN_ID}}"
...
append_summary() {
  local label="$1"
  local topk="$2"
  local actual_avg_sum_bits="$3"
  local status="$4"
  local wikitext2="$5"
  local c4="$6"
  local calibration_seconds="$7"
  local duquant_seconds="$8"
  local inference_wikitext2_seconds="$9"
  local inference_c4_seconds="${10}"
  local plan_path="${11}"
  local output_dir="${12}"
  local plan_log="${13}"
  local run_log="${14}"
...
echo "[ose-topk-routed-tail-2048] output root: $OUT_ROOT"
echo "[ose-topk-routed-tail-2048] test_dataset: $TEST_DATASET"
echo "[ose-topk-routed-tail-2048] tasks: ${TASKS:-<none>}"
echo "[ose-topk-routed-tail-2048] topks: $TOPKS"
echo "[ose-topk-routed-tail-2048] metric: routed_tail"
echo "[ose-topk-routed-tail-2048] tier_quants: $TIER_QUANTS"
echo "[ose-topk-routed-tail-2048] tier_fractions: $TIER_FRACTIONS"
echo "[ose-topk-routed-tail-2048] weight_channel_group_size: $WEIGHT_CHANNEL_GROUP_SIZE"
echo "[ose-topk-routed-tail-2048] router_weight_channel_group_size: $ROUTER_WEIGHT_CHANNEL_GROUP_SIZE"

for idx in "${!TOPK_LIST[@]}"; do
  topk="$(echo "${TOPK_LIST[$idx]}" | xargs)"
  [[ -n "$topk" ]] || continue
  case_num=$((idx + 1))
  label="topk_${topk}"
  ...
  echo
  echo "[ose-topk-routed-tail-2048] case $case_num/$total: $label"
  ...
  append_summary "$label" "$topk" "$actual_avg_sum_bits" "$status" \
  ...
  echo "[ose-topk-routed-tail-2048] finished $label with status=$status"
  echo "[ose-topk-routed-tail-2048] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[ose-topk-routed-tail-2048] summary: $SUMMARY_CSV"
done

echo
echo "[ose-topk-routed-tail-2048] all done: $SUMMARY_CSV"
