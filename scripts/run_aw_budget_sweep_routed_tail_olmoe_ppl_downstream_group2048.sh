...
REPRO_SCRIPT="${REPRO_SCRIPT:-scripts/reproduce_olmoe_691.sh}"
CACHE_DIR="${CACHE_DIR:-$ROOT/cache}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/aw_budget_sweep_routed_tail_olmoe_group2048_${RUN_ID}}"
TEST_DATASET="${TEST_DATASET:-wikitext2,c4}"
DOWNSTREAM_TASKS="${DOWNSTREAM_TASKS:-piqa,winogrande,arc_easy,arc_challenge,openbookqa,boolq}"
...
EST_SECONDS_PER_PPL="${EST_SECONDS_PER_PPL:-1500}"
EST_SECONDS_PER_DOWNSTREAM="${EST_SECONDS_PER_DOWNSTREAM:-10800}"
...
printf 'label,target_avg_sum_bits,run_downstream,tier_quants,tier_fractions,metric,moe_outlier_topk,moe_outlier_quant,moe_outlier_score,weight_channel_group_size,router_weight_channel_group_size,status,wikitext2,c4,acc_avg,arc_challenge,arc_easy,boolq,openbookqa,piqa,winogrande,actual_avg_sum_bits,plan_path,output_dir,plan_log,run_log\n' > "$SUMMARY_CSV"

# Format:
# label|target_avg_sum_bits|run_downstream|tier_quants|tier_fractions
CASES=(
  "aw10|10|0|w4g-1-a4g-1,w4g-1-a8g-1|0.5,0.5"
  "aw11|11|1|w4g-1-a4g-1,w4g-1-a8g-1|0.25,0.75"
  "aw12|12|0|w4g-1-a4g-1,w5g-1-a8g-1,w6g-1-a8g-1|0.234375,0.59375,0.171875"
  "aw13|13|1|w4g-1-a4g-1,w5g-1-a8g-1,w6g-1-a8g-1|0.0625,0.625,0.3125"
  "aw14|14|0|w6g-1-a8g-1,w6g-1-a8g-1|1.0,0.0"
  "aw15|15|0|w6g-1-a8g-1,w8g-1-a8g-1|0.5,0.5"
  "aw16|16|0|w8g-1-a8g-1,w8g-1-a8g-1|1.0,0.0"
)
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[aw-budget-sweep] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
...
append_summary() {
  ...
  local tier_quants="$4"
  local tier_fractions="$5"
  local status="$6"
  local actual_avg_sum_bits="$7"
  local plan_path="$8"
  local output_dir="$9"
  local plan_log="${10}"
  local run_log="${11}"
...

  {
    ...
    csv_quote "$run_downstream"; printf ','
...
echo "[aw-budget-sweep] output root: $OUT_ROOT"
echo "[aw-budget-sweep] model: $MODEL_NAME $MODEL"
echo "[aw-budget-sweep] group size: $WEIGHT_CHANNEL_GROUP_SIZE"
echo "[aw-budget-sweep] all cases run PPL on: $TEST_DATASET"
echo "[aw-budget-sweep] downstream only for A+W=11 and A+W=13: $DOWNSTREAM_TASKS"
echo "[aw-budget-sweep] OSE: topk=$MOE_OUTLIER_TOPK quant=$MOE_OUTLIER_QUANT score=$MOE_OUTLIER_SCORE"

for idx in "${!CASES[@]}"; do
  IFS='|' read -r label target_avg_sum_bits run_downstream tier_quants tier_fractions <<< "${CASES[$idx]}"
  case_num=$((idx + 1))
  ...
  tasks_for_case=""
  est_case="$EST_SECONDS_PER_PPL"
  if [[ "$run_downstream" == "1" ]]; then
    tasks_for_case="$DOWNSTREAM_TASKS"
    est_case="$EST_SECONDS_PER_DOWNSTREAM"
  fi

  mkdir -p "$plan_dir" "$output_dir"

  echo
  echo "[aw-budget-sweep] case $case_num/$total: $label target_avg_sum_bits=$target_avg_sum_bits downstream=$run_downstream"
  echo "[aw-budget-sweep] tier_quants=$tier_quants"
  echo "[aw-budget-sweep] tier_fractions=$tier_fractions"
  ...
  append_summary "$label" "$target_avg_sum_bits" "$run_downstream" "$tier_quants" "$tier_fractions" \
    "$status" "$actual_avg_sum_bits" "$plan_path" "$output_dir" "$plan_log" "$run_log"

  now_ts="$(date +%s)"
  elapsed=$((now_ts - start_ts))
  remaining_cases=$((total - case_num))
  # Remaining estimate is coarse because downstream cases are longer.
  est_remaining=0
  for j in $(seq $((idx + 1)) $((total - 1))); do
    IFS='|' read -r _ _ remaining_downstream _ _ <<< "${CASES[$j]}"
    if [[ "$remaining_downstream" == "1" ]]; then
      est_remaining=$((est_remaining + EST_SECONDS_PER_DOWNSTREAM))
    else
      est_remaining=$((est_remaining + EST_SECONDS_PER_PPL))
    fi
  done
  eta="$(date -d "@$((now_ts + est_remaining))" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || true)"
  echo "[aw-budget-sweep] finished $label with status=$status"
  echo "[aw-budget-sweep] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[aw-budget-sweep] summary: $SUMMARY_CSV"
done

echo
echo "[aw-budget-sweep] all done: $SUMMARY_CSV"
