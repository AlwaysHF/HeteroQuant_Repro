...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/tier_set_route_range_olmoe_ppl_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[tier-route-range] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
  cp -a "$FALLBACK_CACHE_DIR/$planner_cache_name" "$CACHE_DIR/"
fi

mkdir -p "$OUT_ROOT"
SUMMARY_CSV="$OUT_ROOT/summary.csv"
printf 'label,tier_quants,tier_fractions,actual_avg_sum_bits,status,wikitext2,c4,calibration_seconds,duquant_seconds,inference_wikitext2_seconds,inference_c4_seconds,plan_path,output_dir,case_log\n' > "$SUMMARY_CSV"

# All cases keep target_avg_sum_bits=12.0. Costs are A+W sum bits:
...
echo "[tier-route-range] output root: $OUT_ROOT"
echo "[tier-route-range] test_dataset: $TEST_DATASET"
echo "[tier-route-range] tasks: ${TASKS:-<none>}"
echo "[tier-route-range] formula: route_range = (input_range + down_range + weight_range) * route_norm"
...
for idx in "${!CASES[@]}"; do
  ...
  echo
  echo "[tier-route-range] case $case_num/$total: $label"
  echo "[tier-route-range] tier_quants: $tier_quants"
  echo "[tier-route-range] tier_fractions: $tier_fractions"
  ...
  echo "[tier-route-range] finished $label with status=$status"
  echo "[tier-route-range] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[tier-route-range] summary: $SUMMARY_CSV"
done

echo
echo "[tier-route-range] all done: $SUMMARY_CSV"
