...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/ose_score_routed_tail_olmoe_ppl_group2048_${RUN_ID}}"
...
SCORES="${SCORES:-smooth_scale,weight_max,weight_error}"
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[ose-score-routed-tail-2048] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
  cp -a "$FALLBACK_CACHE_DIR/$planner_cache_name" "$CACHE_DIR/"
fi

mkdir -p "$OUT_ROOT"
SUMMARY_CSV="$OUT_ROOT/summary.csv"
printf 'label,moe_outlier_score,moe_outlier_topk,moe_outlier_quant,metric,tier_quants,tier_fractions,weight_channel_group_size,router_weight_channel_group_size,actual_avg_sum_bits,status,wikitext2,c4,calibration_seconds,duquant_seconds,inference_wikitext2_seconds,inference_c4_seconds,plan_path,output_dir,plan_log,run_log\n' > "$SUMMARY_CSV"

IFS=',' read -r -a SCORE_LIST <<< "$SCORES"
...
append_summary() {
  local label="$1" score="$2" actual_avg_sum_bits="$3" status="$4" wikitext2="$5" c4="$6"
  local calibration_seconds="$7" duquant_seconds="$8" inference_wikitext2_seconds="$9"
  local inference_c4_seconds="${10}" plan_path="${11}" output_dir="${12}" plan_log="${13}" run_log="${14}"
  {
...
echo "[ose-score-routed-tail-2048] output root: $OUT_ROOT"
echo "[ose-score-routed-tail-2048] scores: $SCORES"
echo "[ose-score-routed-tail-2048] metric: routed_tail"
echo "[ose-score-routed-tail-2048] topk: $MOE_OUTLIER_TOPK"
echo "[ose-score-routed-tail-2048] quant: $MOE_OUTLIER_QUANT"
echo "[ose-score-routed-tail-2048] tier: $TIER_QUANTS / $TIER_FRACTIONS"

for idx in "${!SCORE_LIST[@]}"; do
  score="$(echo "${SCORE_LIST[$idx]}" | xargs)"
  [[ -n "$score" ]] || continue
  case_num=$((idx + 1))
  label="score_${score}"
  ...
  echo
  echo "[ose-score-routed-tail-2048] case $case_num/$total: $label"
  ...
  append_summary "$label" "$score" "$actual_avg_sum_bits" "$status" "$wikitext2" "$c4" \
  ...
  echo "[ose-score-routed-tail-2048] finished $label with status=$status"
  echo "[ose-score-routed-tail-2048] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[ose-score-routed-tail-2048] summary: $SUMMARY_CSV"
done

echo
echo "[ose-score-routed-tail-2048] all done: $SUMMARY_CSV"
