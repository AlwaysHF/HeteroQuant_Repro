...
OUT_ROOT="${OUT_ROOT:-$ROOT/experiments/route_range_vs_range_olmoe_ppl_$(date +%Y%m%d_%H%M%S)}"
...
if [[ ! -f "$CACHE_DIR/$planner_cache_name" && -f "$FALLBACK_CACHE_DIR/$planner_cache_name" ]]; then
  echo "[route-range-vs-range] copy cached planner dataloader from $FALLBACK_CACHE_DIR/$planner_cache_name"
  cp -a "$FALLBACK_CACHE_DIR/$planner_cache_name" "$CACHE_DIR/"
fi

mkdir -p "$OUT_ROOT"
SUMMARY_CSV="$OUT_ROOT/summary.csv"
printf 'label,route_alpha,score_transform,input_score_coef,down_score_coef,weight_score_coef,weight_score_projs,tier_quants,tier_fractions,actual_avg_sum_bits,status,wikitext2,c4,calibration_seconds,duquant_seconds,inference_wikitext2_seconds,inference_c4_seconds,plan_path,output_dir,case_log\n' > "$SUMMARY_CSV"
...
echo "[route-range-vs-range] output root: $OUT_ROOT"
echo "[route-range-vs-range] test_dataset: $TEST_DATASET"
echo "[route-range-vs-range] tasks: ${TASKS:-<none>}"
echo "[route-range-vs-range] route_range: (input_range + down_range + weight_range) * route_norm"
echo "[route-range-vs-range] range: input_range + down_range + weight_range"

for idx in "${!CASES[@]}"; do
  IFS='|' read -r label route_alpha <<< "${CASES[$idx]}"
  case_num=$((idx + 1))
  ...
  echo
  echo "[route-range-vs-range] case $case_num/$total: $label"
  echo "[route-range-vs-range] route_alpha=$route_alpha"
  ...
  append_summary "$label" "$route_alpha" "$actual_avg_sum_bits" "$status" \
  ...
  echo "[route-range-vs-range] finished $label with status=$status"
  echo "[route-range-vs-range] elapsed=${elapsed}s, estimated_remaining=${est_remaining}s, eta=${eta:-unknown}"
  echo "[route-range-vs-range] summary: $SUMMARY_CSV"
done

echo
echo "[route-range-vs-range] all done: $SUMMARY_CSV"
