...
RUN_ROOT="${RUN_ROOT:-$ROOT/experiments/ose_speed_olmoe_${TIMESTAMP}}"
...
run_variant() {
  ...
  local topk="$4"
  local quant="$5"
  local shared_score="$6"
  local completed_runs="$7"
  local completed_seconds="$8"
  ...
  log_master "variant $index/$total start: $label topk=$topk quant=$quant shared_score=$shared_score"
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$label" "$topk" "$quant" "$shared_score" "$exit_code" "$elapsed" \
...
main() {
  log_master "OSE speed ablation root: $RUN_ROOT"
  ...
  printf 'variant,topk,quant,shared_score,exit_code,elapsed_seconds,ppl_wikitext2,duquant_seconds,calibration_seconds,inference_seconds_wikitext2,output_dir,log\n' >"$SUMMARY_CSV"

  local variants=(
    "ose_off|0|same|none"
    "ose_fp16|64|fp16|smooth_scale"
    "ose_w8a8|64|w8a8|smooth_scale"
  )
  if [[ "${INCLUDE_EXTRA:-0}" == "1" ]]; then
    variants+=(
      "ose_w16a16|64|w16a16|smooth_scale"
      "ose_w8a8_topk32|32|w8a8|smooth_scale"
      "ose_w8a8_topk128|128|w8a8|smooth_scale"
    )
  fi
  ...
  for spec in "${variants[@]}"; do
    IFS='|' read -r label topk quant shared_score <<<"$spec"
    run_variant "$i" "$total" "$label" "$topk" "$quant" "$shared_score" "$completed_runs" "$completed_seconds"
